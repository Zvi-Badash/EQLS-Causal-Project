...
def choose_columns() -> pd.DataFrame:
    ...
    df.rename(columns={treatment_column: 'treatment', outcome_column: 'outcome'}, inplace=True)
    return df

def preprocess_data(df: pd.DataFrame,
                    na_threshold: float = 0.5,
                    impute_strategy: Literal['drop', 'mean', 'median'] = 'drop',
                    treatment_dichotomize_value: Union[float, Literal['median']] = 'median') -> pd.DataFrame:
    ...
    if treatment_dichotomize_value == 'median':
        ...
        treatment_threshold = treatment_dichotomize_value
...
if __name__ == "__main__":
    df = choose_columns()
    df = preprocess_data(df, na_threshold=0.5, impute_strategy='drop', treatment_dichotomize_value='median')
